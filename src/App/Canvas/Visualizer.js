...
class Visualizer extends Component {
	...
	onLoad = (evt) => {
		...
		if (xhr.readyState === XMLHttpRequest.DONE) {
			if (xhr.status === 200) {
				...
				const root = stratify(d3.dsvFormat('_').parse(xhr.responseText))
					.sum(d => 1)

				this.setState((prevState) => {
					const loading_status = Object.assign({}, prevState.loading_status, {done: true})
				...
				this.setState((prevState) => {
					const loading_status = Object.assign({}, prevState.loading_status, {
						error: true, error_message: `Error ${xhr.status}, check log statements.`
					})
					return Object.assign({}, prevState, {loading_status: loading_status})
				})
			}
	...
	drawMainPack = (root, nodes, prevTransformations) => {
		...
		node.append("title")
			.text(node => `${node.data.name}\nContains: ${node.children? node.children.length : 0} ${text[node.depth]}`);

		const scaleX = d3.scaleLinear().range([width / 2 - root.r, width / 2 + root.r])
		const scaleY = d3.scaleLinear().range([height / 2 - root.r, height / 2 + root.r])
		...
		const zoomed = (transform) => {
			...
			node.attr('transform', d => `translate(${scaleX(d.x) + x}, ${scaleY(d.y) + y})`)
			circle.attr('r', d => d.r * k)
		}

		svg.call(d3
			.zoom()
			.on('zoom', () => zoomed(d3.event.transform)).scaleExtent([0.75, 25])
		...
		const subject_area_nodes_selection = svg.selectAll('g#main').filter(d => d.depth === 2)
	...
	drawSubPack = ({root, nodes}, sub_root, prevTransformations) => {
		...
		const non_main_circle_selection = svg.selectAll('g#main').filter(d => d.id !== root.data.id)
		const main_circle_selection = svg.selectAll('g#main').filter(d => d.id === root.data.id)
		...
		const selection = svg.select('g#sub_pack').selectAll('g')
		...
		svg.call(d3.zoom().on('zoom', () => zoomed(d3.event.transform)).scaleExtent([0.75, 25]))

		const leaf_nodes = svg.select('g#sub_pack').selectAll('g').filter(d => d.data.id !== sub_root.data.id)
	...
	onProgress = (evt) => {
		if (evt.lengthComputable) {
			...
			this.setState((prevState) => {
				const loading_status = Object.assign({}, prevState.loading_status, {progress: percentComplete})
				return Object.assign({}, prevState, {loading_status: loading_status})
			})
		}
	}
	...
	onError = (evt) => {
		...
		this.setState((prevState) => {
			const loading_status = Object.assign({}, prevState.loading_status, {
				error: true, error_message: 'Error occured, check log statements.'
			})
			return Object.assign({}, prevState, {loading_status: loading_status})
		})
	}
	...
	render() {
		...
		if (done) {
			return (
				<div id='container'>
					<svg id='main_svg' width={window.innerWidth} height={window.innerHeight}/>
					<div id='input'>
